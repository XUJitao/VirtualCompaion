...
\indent Nous avons créer une méthode \emph{setAppointement(GestionCalendar Gcal, String oobContent, String operationType)} qui nous permet de distinguer l'ajout et la suppression du rendez-vous. Pour pouvoir utiliser les méthodes déjà existantes, il faut avoir le titre, la date de début et la date de fin du rendez-vous. Pour simplifier le traitement, nous avons créer une méthode \emph{setBeginTimeAndGetTitle(String oobContent, Calendar beginTime, String operationType)} qui régler la date de début et retourner le titre de rendez-vous en même temps. Dans cette dernière méthode, le paramètre \emph{beginTime} en entrée est la date actuel du système. Nous avons met à défaut toutes les parties de la date commes la date actuel. Nous avons vérifié la chaîne \emph{oobContent} dans l'ordre suivantes : jour, mois, an, heure, minute, titre. Pour chaque partie précisée, nous avons met à jour l'information. Si l'utilisateur n'a pas précisé le titre du rendez-vous, le titre par défaut sera une chaîne vide.

\indent La méthode \emph{setBeginTimeAndGetTitle} permet l'utilisateur de ne pas préciser n'importe quelle partie d'une date tant que les informations précisées sont dans l'ordre.

\subsubsection*{Mettre à jour le prochain rendez-vous}

\indent Nous avons aussi supprimé la méthode \emph{prochainRDV} pour créer une nouvelle dans la classe \emph{ToolManager}. La méthode \emph{getNextEvent()} permet de retourner une chaîne de caractère qui contient les informations du prochain rendez-vous. Dans la méthode, nous avons créer un instance de \emph{Cursor} qui peut se déplacer comme un pointeur. Nous cherchions dans la liste d'événement stocké dans l'agenda de l'appareil pour trouver l'événement le plus proche de la date actuel.

\subsection{Ajout plusieurs catégories de questions/réponses sur le site Pandorabots}

\indent Nous continuions à utiliser le compte créé par les étudiants précédents pour ajouter des catégories de questions/réponses, avec les identifiants suivants:

\begin{center}
	Email : \emph{alexandre.levacher@insa-rouen.fr}\\
	Mot de passe : \emph{PAOCompagnonVirtuel3}
\end{center}

\indent Tout les informations d'une quesion/réponse sont comprises entre \emph{<category>} et \emph{</cagegory>}. La balise \emph{<pattern>} contient la question et la balise \emph{<template>} est la partie de réponse intelligente. La balise \emph{<oob>} se situe dans la balise \emph{<template>}.
...
\indent Premièrement, nous avons séparé le centre de l'interface d'utilisateur à deux partie. La partie gauche affiche les cinq reconnaissaces vocales du voix de l'utilisateur plus possibles. La partie droite affiche la réponse de notre application s'il existe. En bas de l'écran, nous avons ajouté un \emph{textfield} pour afficher le prochain événement dans le calendrier. Ce \emph{textfield} va se mettre à jour s'il existe un changement d'événement plus proche dans le calendrier grâce à la fonction \emph{getNextEvent}. Autrement, nous avons changé le personnage d'animation. Nous allons parler dans la paragraphe suivante.
